                  ...
                  <tr v-for="(order, i) in this.data" :key="order.order_id" @click="(order_id => {visitOrder(order_id);})(order.order_id)">
                    ...
                    <td class="order__time">
                      {{(time => {
                        ...
                        return `${date.getDate().toString().padStart(2, '0')}-${date.getMonth().toString().padStart(2, '0')}-${date.getFullYear().toString().padStart(2, '0')} pukul ${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}:${date.getSeconds().toString().padStart(2, '0')}`;
                      })(order.created_at)}}
                    </td>
                  </tr>
                </tbody>
